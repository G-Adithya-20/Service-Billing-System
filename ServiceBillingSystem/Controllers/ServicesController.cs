...
public class ServicesController : Controller
{
    ...
    [HttpPost] //runs when user submites create form
    ...
    public IActionResult Search(string term) //Services/Search?term=
    {
        ...
        var services = _context.Services.Where(x => x.IsActive && x.Name.Contains(term)).
        Select(x => new
            {
                id = x.Id,
                name = x.Name,
                price = x.Price
            }).Take(10).ToList(); //max 10 matching services
